...
public class ColdStorageHelper {
    ...
    public static final String COLD_STORAGE_BEIGN_RETRIEVED_PROPERTY = "coldstorage:beingRetrieved";
     ...
     *          can end up with an expected behaviour.
     ...
     *             retrieved
     */
    public static DocumentModel retrieveFromColdStorage(CoreSession session, DocumentRef documentRef) {
        DocumentModel documentModel = session.getDocument(documentRef);
        boolean hasFacet = documentModel.hasFacet(COLD_STORAGE);
        if (!hasFacet || (hasFacet && isNull(documentModel.getPropertyValue(COLD_STORAGE_CONTENT_PROPERTY)))) {
            throw new NuxeoException(String.format("No cold storage content defined for document: %s.", documentModel),
                    SC_NOT_FOUND);
        }

        Serializable beingRetrieved = documentModel.getPropertyValue(COLD_STORAGE_BEIGN_RETRIEVED_PROPERTY);
        ...
        documentModel.setPropertyValue(COLD_STORAGE_BEIGN_RETRIEVED_PROPERTY, true);
        return session.saveDocument(documentModel);
    }
